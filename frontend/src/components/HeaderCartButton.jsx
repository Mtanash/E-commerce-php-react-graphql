...
export default function HeaderCartButton() {
  const { getTotalItems, cartIsOpen, toggleCart } = useCart();
  const cartTotal = getTotalItems();

  const [cartRoot, setCartRoot] = useState(null);

  useEffect(() => {
    const cartRootContainer = document.createElement("div");
    cartRootContainer.id = "cart-root";
    document.body.appendChild(cartRootContainer);
    setCartRoot(cartRootContainer);

    return () => {
      document.body.removeChild(cartRootContainer);
    };
  }, []);
  ...
  return (
    <>
      <button
        className={`${classes.cartButton} cart-button`}
        onClick={toggleCart}
      >
        {cartTotal > 0 && (
          <span className={classes.cartTotal}>{cartTotal}</span>
        )}
      ...
      {cartIsOpen &&
        ...
        ReactDOM.createPortal(
          <CartOverlay toggleCart={toggleCart} cartIsOpen={cartIsOpen} />,
          cartRoot
        )}
    </>
  );
